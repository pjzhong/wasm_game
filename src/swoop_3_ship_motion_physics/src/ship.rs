...
const ANGULAR_DEAMPING: f32 = 8.0;
...
impl Ship {
    ...
    pub fn update(&mut self, dt: f32) {
        ...
        acceleration.2 -= self.velocity.rot * ANGULAR_DEAMPING;
        ...
        if 0.0 < self.linear_thrust {
            self.linear_thrust -= 0.01;
        } else if self.linear_thrust < 0.0 {
            self.linear_thrust += 0.01;
        }

        if 0.0 < self.angular_thrust {
            self.angular_thrust -= 0.1;
        } else if self.angular_thrust < 0.0 {
            self.angular_thrust += 0.1;
        }
    }
}
